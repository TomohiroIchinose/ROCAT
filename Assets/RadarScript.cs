...
public class RadarScript : MonoBehaviour {
    ...
    void OnGUI()
    {
        ...
        GUI.DrawTexture(new Rect(mapCenter.x - 100, mapCenter.y - 100, 200, 200), radarBG);
        GUI.DrawTexture(new Rect(mapCenter.x - 10, mapCenter.y - 10, 20, 20), MyPosition);
    ...
    private void drawBlip(GameObject go, Texture aTexture)
    {
        ...
        //Debug.Log(go.name + ":" + dist);

        if (dist <= maxDist)
        {
            // this is the diameter of our largest radar circle
            GUI.DrawTexture(new Rect(mapCenter.x + bX - 10, mapCenter.y + bY - 10, 20, 20), aTexture);
        }
